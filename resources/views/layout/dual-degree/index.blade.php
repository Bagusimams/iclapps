...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active">Dual Degree List</li>
  </ol>
</section>
  ...
  <div class="box">
    <div class="box-body">
      <div class="col-sm-8">
          ...
          <div class="col-sm-5">
            {!! Form::select('university_id', getJointUniversities(), $university_id, ['class' => 'form-control', 'style'=>'width: 100%', 'id' => 'university_id', 'onchange' => 'advanceSearch()']) !!}
          </div>
          ...
          @if($role == 'staff')
            <a href="{{ url($role. '/dual-degree/' . $university_id . '/upload') }}" class="btn btn-success">Upload</a>
          @endif
      </div>
      <div class="col-sm-4">
      ...
      @if(Auth::guard('lecturer')->user() && isset($pre))
        <div class="col-sm-8">
          @include('layout.dual-degree.pre')
        </div>
      @endif
        ...
        <tbody id="table-course">
          @foreach($exchanges as $exchange)
            <tr>
              ...
              <td>{{ $exchange->university_joint->name }}</td>
              @if(Auth::guard('staff')->user())
                <td><a href="{{ url('/' . $role. '/dual-degree/' . $exchange->id . '/important') }}"><i class="fa fa-hand-o-right orange"></i></a></td>
                <td><a href="{{ url('/' . $role. '/dual-degree/' . $exchange->id . '/assign') }}"><i class="fa fa-hand-o-right pink"></i></a></td>
                <td><a href="{{ url('/' . $role. '/dual-degree/' . $exchange->id . '/detail') }}"><i class="fa fa-hand-o-right tosca"></i></a></td>
                <td>
                    ...
                    @include('layout.deleteModal', ['id' => $exchange->id, 'data' => 'Dual Degree', 'formName' => 'delete-form-' . $exchange->id])

                    <form id="delete-form-{{$exchange->id}}" action="{{ url('/' . $role . '/dual-degree/' . $exchange->id . '/delete') }}" method="POST" style="display: none;">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                    </form>
                </td>
              @elseif(Auth::guard('lecturer')->user())
                <td>
                  <form action="{{ url('/' . $role . '/dual-degree/' . $exchange->id . '/report') }}" method="POST">
...
@section('javascript-addon') 
  <script type="text/javascript">
    ...
    function advanceSearch()
    {
      ...
      window.location   = window.location.origin + '/{{ $role }}/dual-degree/' + university_id + '/{{ $status }}/' + show;
    }
  </script>
@endsection 