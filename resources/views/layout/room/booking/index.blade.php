...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active">Room Bookings List</li>
  </ol>
</section>
...
<section class="content">             
  <div class="box">
    <div class="box-body">
      <div class="col-sm-8">
          <a href="{{ url('/' . $role . '/room/booking/create') }}" class="btn btn-success col-sm-4">Add Room Booking</a>
        ...
        <tbody id="table-course">
          @foreach($bookings as $booking)
            <tr>
              ...
              <td><a href="{{ url('/' . $role. '/room/booking/' . $booking->id . '/detail') }}"><i class="fa fa-hand-o-right tosca"></i></a></td>
              @if(Auth::guard('staff')->user())
                <td><a href="{{ url('/' . $role. '/room/booking/' . $booking->id . '/change/1') }}"><i class="fa fa-check @if($booking->isApproved == 1) tosca @else gray @endif" aria-hidden="true"></i></a></td>
                <td><a href="{{ url('/' . $role. '/room/booking/' . $booking->id . '/change/2') }}"><i class="fa fa-ban @if($booking->isApproved == 2) red @else gray @endif" aria-hidden="true"></i></a></td>
                <td><a href="{{ url('/' . $role. '/room/booking/' . $booking->id . '/edit') }}"><i class="fa fa-pencil-square-o orange"></i></a></td>
                <td>
                    ...
                    @include('layout.deleteModal', ['id' => $booking->id, 'data' => 'room booking', 'formName' => 'delete-form-' . $booking->id])

                    <form id="delete-form-{{$booking->id}}" action="{{ url('/' . $role . '/room/booking/' . $booking->id . '/delete') }}" method="POST" style="display: none;">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                    </form>
                </td>
...
@section('javascript-addon') 
  <script type="text/javascript">
    ...
    {
      var show       = $('#show').val();   
      window.location = window.location.origin + '/{{ $role }}/room/booking/' + show;
    }
  </script>
@endsection 