...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active">Variable List</li>
  </ol>
</section>
        ...
        <tbody id="table-course">
          @foreach($variables as $variable)
            <tr>
              ...
              <td>{{ $variable->score }}</td>
              @if(Auth::guard('staff')->user())
                <td><a href="{{ url('/' . $role. '/variable/' . $variable->id . '/edit') }}"><i class="fa fa-pencil-square-o orange"></i></a></td>
              @endif
            </tr>
          @endforeach
        </tbody>
...
@section('javascript-addon') 
  <script type="text/javascript">
    ...
    {
      var show       = $('#show').val();   
      window.location = window.location.origin + '/{{ $role }}/variable/' + show;
    }
  </script>
@endsection 