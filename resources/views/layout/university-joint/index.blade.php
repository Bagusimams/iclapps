...
<section class="content">             
  <div class="box">
    <div class="box-body">
      <div class="col-sm-8">
          <a href="{{ url('/' . $role . '/university-joint/create') }}" class="btn btn-success col-sm-4">Add University Joint</a>
        ...
        <tbody id="table-course">
          @foreach($universities as $university)
            <tr>
              ...
              <td><a href="{{ url('/' . $role. '/university-joint/' . $university->id . '/edit') }}"><i class="fa fa-pencil-square-o orange"></i></a></td>
              <td>
                  ...
                  @include('layout.deleteModal', ['id' => $university->id, 'data' => 'University Joint', 'formName' => 'delete-form-' . $university->id])

                  <form id="delete-form-{{$university->id}}" action="{{ url('/' . $role . '/university-joint/' . $university->id . '/delete') }}" method="POST" style="display: none;">
                      {{ csrf_field() }}
                      {{ method_field('DELETE') }}
                  </form></td>
            </tr>
...
@section('javascript-addon') 
  <script type="text/javascript">
    ...
    {
      var show       = $('#show').val();   
      window.location = window.location.origin + '/{{ $role }}/university-joint/' + show;
    }
  </script>
@endsection 