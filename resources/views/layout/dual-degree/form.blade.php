<div class="panel-body">
    <div class="row">
        ...
        <div class="form-group">
            {!! Form::label('university_joint_id', 'University Dual Degree', array('class' => 'col-sm-2 control-label')) !!}
            <div class="col-sm-3">
                @if($SubmitButtonText != 'Tambah')
                    {!! Form::text('university_joint', $exchange->university_joint->name, array('class' => 'form-control', 'readonly' => 'readonly')) !!}
                @else
                    {!! Form::select('university_joint_id', getJointUniversities(), null, ['class' => 'form-control select2', 'style'=>'width: 100%']) !!}
                @endif
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                @if($SubmitButtonText == 'View')
                    {!! Form::text('semester_applied', null, array('class' => 'form-control', 'readonly' => 'readonly')) !!}
                @else
                    {!! Form::text('semester_applied', null, array('class' => 'form-control')) !!}
                @endif
            </div>
        </div>
...
@elseif($SubmitButtonText == 'View')
    @if(Auth::guard('student')->user())
        @if($exchange->final_report == null)
            <a href="{{ url('/' . $role . '/dual-degree/edit') }}" class="btn btn-success btn-flat btn-block form-control">Edit Dual Degree Form</a>
        @endif
    @else
        @if($exchange->final_report == null)
            <a href="{{ url('/' . $role . '/dual-degree/' . $exchange->id . '/edit') }}" class="btn btn-success btn-flat btn-block form-control">Edit Dual Degree Form</a>
        @endif
        @if($exchange->is_form_complete == 0)
            ...
            <a href="{{ url('/' . $role . '/dual-degree/' . $exchange->id . '/reject') }}" class="btn btn-danger btn-flat btn-block form-control">Reject Dual Degree Form</a>
        @elseif($exchange->is_ticket_complete == 0 && $exchange->is_form_complete == 1)
            <a href="{{ url('/' . $role . '/dual-degree/' . $exchange->id . '/ticket') }}" class="btn btn-warning btn-flat btn-block form-control">Accept Dual Degree Ticket</a>
        @elseif($exchange->is_payment_complete == 0 && $exchange->is_form_complete == 1)
            <a href="{{ url('/' . $role . '/dual-degree/' . $exchange->id . '/payment') }}" class="btn btn-danger btn-flat btn-block form-control">Accept Dual Degree Payment</a>
        @endif
    @endif
@endif
