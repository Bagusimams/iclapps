            ...
            <tr>
              ...
              @if(Auth::guard('staff')->user())
                ...
                <td>
                    ...
                    @include('layout.deleteModal', ['id' => $inventory->id, 'data' => 'inventory', 'formName' => 'delete-form-' . $inventory->id])

                    <form id="delete-form-{{$inventory->id}}" action="{{ url('/' . $role . '/inventory/' . $inventory->id . '/delete') }}" method="POST" style="display: none;">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                    </form>
                </td>
